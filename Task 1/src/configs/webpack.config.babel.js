...
const listHtml = paths.list.html.filtered.map((i) => {
...
const commonConfig = (argv) => ({
	...
	entry: [...paths.list.html.twigs, paths.file.index.js, paths.file.index.scss],
	output: {
		clean: true,
		filename: argv.env["path-build"]
			? paths.file.name.js.replace("assets/", argv.env["path-build"]).slice(1)
			: paths.file.name.js,
	...
	resolve: {
		roots: [path.resolve(paths.dir.root, "src")],
		alias: {},
	},
	optimization: {
		splitChunks: {
	...
	plugins: [
		...
		new CopyPlugin({
			patterns: [
				{
					from: "./static",
					to: argv.env["path-build"]
						? `.${argv.env["path-build"]}`
						: path.resolve(paths.dir.root, "dist"),
				},
			],
		}),
		...htmlPlugins,
		...
		new MiniCssExtractPlugin({
			filename: argv.env["path-build"]
				? paths.file.name.css.replace(
						"assets/",
						argv.env["path-build"].slice(1)
				  )
				: paths.file.name.css,
		}),
	],
